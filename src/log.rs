...
/// so even oxid is fine with 1.31+ rustc version, we need to use local_inner_macros as well
#[doc(hidden)]
#[macro_export]
...
pub fn __private_api_log_lit(
    ...
    let log_fn = match level {
        Level::Debug => oxid_wasm::console_debug,
        Level::Warn => oxid_wasm::console_warn,
        Level::Info => oxid_wasm::console_info,
        Level::Trace => oxid_wasm::console_debug,
        Level::Error => oxid_wasm::console_error,
    };
...
pub fn __private_api_log_lit(
    ...
    unsafe { oxid_android::oxid_android_log(msg.as_ptr()) };
}
