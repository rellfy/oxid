...
pub fn main(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    while let Some(TokenTree::Punct(punct)) = source.peek() {
        ...
        if attribute_name == "oxid_framework" {
    ...
    if let TokenTree::Ident(ident) = source.next().unwrap() {
        ...
        panic!("[oxid_framework::main] is allowed only for async functions");
    }
    ...
    if let TokenTree::Ident(ident) = source.next().unwrap() {
        ...
        panic!("[oxid_framework::main] is allowed only for functions");
    }
    ...
    if let TokenTree::Ident(ident) = source.next().unwrap() {
        ...
        panic!("[oxid_framework::main] expecting main function");
    }
    ...
    ",
        crate_name = crate_rename.unwrap_or_else(|| "oxid_framework".to_string()),
        method = method,
        ident = ident
    )
    .parse()
