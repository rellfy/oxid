...
const OXID_EVENT_TYPE_TOUCHES_BEGAN = 10;
const OXID_EVENT_TYPE_TOUCHES_MOVED = 11;
const OXID_EVENT_TYPE_TOUCHES_ENDED = 12;
const OXID_EVENT_TYPE_TOUCHES_CANCELLED = 13;
...
into_oxid_mousebutton = function (btn) {
    switch (btn) {
        case 0: return 0;
        case 1: return 2;
        case 2: return 1;
...
into_oxid_keycode = function (key_code) {
    switch (key_code) {
    ...
    env: {
        ...
        init_opengl: function (ptr) {
            ...
            canvas.onkeydown = function (event) {
                ...
                wasm_exports.key_down(oxid_key_code, modifiers, event.repeat);
                // for "space" preventDefault will prevent
                // key_press event, so send it here instead
                if (oxid_key_code == 32) {
                    wasm_exports.key_press(oxid_key_code);
                }
            };
            canvas.onkeyup = function (event) {
                var oxid_key_code = into_oxid_keycode(event.code);
                wasm_exports.key_up(oxid_key_code);
            };
            ...
            canvas.addEventListener("touchstart", function (event) {
                ...
                for (touch of event.changedTouches) {
                    wasm_exports.touch(OXID_EVENT_TYPE_TOUCHES_BEGAN, touch.identifier, Math.floor(touch.clientX), Math.floor(touch.clientY));
                }
            });
            canvas.addEventListener("touchend", function (event) {
                ...
                for (touch of event.changedTouches) {
                    wasm_exports.touch(OXID_EVENT_TYPE_TOUCHES_ENDED, touch.identifier, Math.floor(touch.clientX), Math.floor(touch.clientY));
                }
            });
            canvas.addEventListener("touchcancel", function (event) {
                ...
                for (touch of event.changedTouches) {
                    wasm_exports.touch(OXID_EVENT_TYPE_TOUCHES_CANCELED, touch.identifier, Math.floor(touch.clientX), Math.floor(touch.clientY));
                }
            });
            canvas.addEventListener("touchmove", function (event) {
                ...
                for (touch of event.changedTouches) {
                    wasm_exports.touch(OXID_EVENT_TYPE_TOUCHES_MOVED, touch.identifier, Math.floor(touch.clientX), Math.floor(touch.clientY));
                }
            });
...
function oxid_add_plugin(plugin) {
    plugins.push(plugin);
}
